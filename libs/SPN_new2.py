...
class spn_block(nn.Module):
    ...
    def forward(self,x,G1,G2,G3):
        ...
        sum_abs[sum_abs.data == 0] = 1e-6
        # mask_need_norm = sum_abs.ge(1)
        # mask_need_norm = mask_need_norm.float()
        G1 = torch.div(G1, sum_abs)
        G2 = torch.div(G2, sum_abs)
        G3 = torch.div(G3, sum_abs)
...
class SPN(nn.Module):
    def __init__(self,vggPath=None):
        super(SPN,self).__init__()
        # conv for mask
        self.mask_conv = nn.Conv2d(21,32,4,2,1)
